...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/16 18:56:49 by omakovsk          #+#    #+#             */
/*   Updated: 2019/02/16 18:56:50 by omakovsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_print_show(t_crypt_info *crypt_info)
{
	...
	{
		ft_printf("salt=");
		ft_print_hex(crypt_info->flags.salt, 8);
	}
	if (crypt_info->flags.k && (crypt_info->cryptptr == des ||
		crypt_info->cryptptr == des_cbc || crypt_info->cryptptr == des_ecb))
	{
		ft_printf("key =");
		ft_print_hex(crypt_info->flags.key, 8);
	}
	else
	{
		ft_printf("key =");
		ft_print_hex(crypt_info->flags.key, 24);
	}
	...
	{
		ft_printf("iv  =");
		ft_print_hex(crypt_info->flags.in_vector, 8);
	}
}
